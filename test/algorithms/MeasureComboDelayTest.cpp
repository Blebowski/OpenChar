...
void test_inv(Context &ctx, Algorithms &algs)
{
    ...
    assert (EQUAL_WITH_TOL(arc.GetFallDelays()[0][0], 0.18410));
    assert (EQUAL_WITH_TOL(arc.GetFallDelays()[0][1], 2.72870));
    assert (EQUAL_WITH_TOL(arc.GetFallDelays()[1][0], 0.20790));
    assert (EQUAL_WITH_TOL(arc.GetFallDelays()[1][1], 2.75010));

    assert (EQUAL_WITH_TOL(arc.GetRiseDelays()[0][0], 0.09330));
    assert (EQUAL_WITH_TOL(arc.GetRiseDelays()[0][1], 1.36650));
    assert (EQUAL_WITH_TOL(arc.GetRiseDelays()[1][0], 0.11510));
    assert (EQUAL_WITH_TOL(arc.GetRiseDelays()[1][1], 1.38750));
}
